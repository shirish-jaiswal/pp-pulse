// import { sb_floatResult } from "@/features/round-details/components/game-metadata/game-result/result-sheets/sweet-bonanza/rules";
// import { getBaccaratWinnerFromEvents } from "@/features/round-details/components/round-audit/tab-content/card-details/baccarat/baccarat-hand-report-rules";
// import { RoundMetaInfo } from "@/features/round-details/types/game-details";
// import { CardDetailsInfo } from "@/features/round-details/types/card-details";
// import { FloatingGameResult } from "./core/types";

// type HandlerParams = {
//     gameDetails: RoundMetaInfo;
//     cardDetails: CardDetailsInfo;
//     extraData?: any;
// };

// export const GameHandlers: Record<string, (params: HandlerParams) => FloatingGameResult> = {
//     "sweet-bonanza": ({ gameDetails }) => {
//         const sbData = sb_floatResult(gameDetails);
//         return {
//             label: "BONUS",
//             imgurl: sbData || "",
//             className: "border-pink-500 bg-pink-500/10",
//             textClassName: "text-white",
//         };
//     },

//     "roulette": ({ gameDetails }) => {
//         const raw = (gameDetails?.[0]?.Description || "").toString().toLowerCase();
//         let colorKey: "red" | "black" | "tie" = "tie";

//         if (raw.includes("red")) colorKey = "red";
//         else if (raw.includes("black")) colorKey = "black";

//         const number = raw.match(/\d+/)?.[0];
//         return {
//             label: number || colorKey.toUpperCase(),
//             className: styleMap[`roulette-${colorKey}`] || styleMap.default,
//             textClassName: "text-white",
//         };
//     },

//     "baccarat": ({ cardDetails, extraData }) => {
//         let winner = getBaccaratWinnerFromEvents(cardDetails, extraData || []);
//         if (winner === "PLAYER") {
//             winner = "PLR";
//         } else if (winner === "BANKER") {
//             winner = "BNK";
//         } else {
//             winner = "TIE";
//         }
//         return {
//             label: winner,
//             className:
//                 winner === "PLR" ? styleMap["baccarat-player"] :
//                     winner === "BNK" ? styleMap["baccarat-banker"] :
//                         styleMap["roulette-tie"],
//             textClassName: "text-white",
//         };
//     },

//     "crash-game": ({ gameDetails }) => {
//         console.log("Processing crash game details:", gameDetails);
//         const raw = (gameDetails?.[0]?.state_indicator / 100 || "").toString().toLowerCase();
//         const multiplier = raw +"x";
//         return {
//             label: multiplier,
//             className: styleMap.crash,
//             textClassName: "text-white",
//         };
//     },

//     "game-show": ({ gameDetails }) => {
//         const raw = (gameDetails?.[0]?.Description || "").toString().toLowerCase();
//         return {
//             label: raw,
//             className: "bg-gradient-to-r from-yellow-400 to-yellow-600 border-yellow-700",
//             textClassName: "text-white",
//         };
//     },

//     "treasure-island": ({ gameDetails }) => {
//         const raw = (gameDetails?.[0]?.multiplier || "").toString().toLowerCase();
//         return {
//             label: raw,
//             className: "bg-gradient-to-r from-green-400 to-green-600 border-green-700",
//             textClassName: "text-white",
//         };
//     }
// };