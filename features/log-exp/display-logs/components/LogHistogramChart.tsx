...
export function LogHistogramChart({ data, isVisible }: LogHistogramChartProps) {
  ...
  // Generate exactly 7 evenly spaced label points across the dataset length array
  const renderSevenAxisTicks = () => {
    const totalItems = data.length;

    // Fallback if data size is small
    if (totalItems < 7) {
      return data.map((bucket, idx) => (
        <span key={idx}>
          {new Date(bucket.key_as_string).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit", second: "2-digit" })}
        </span>
      ));
    }

    return Array.from({ length: 7 }).map((_, index) => {
      // Find fractional target idx position (0/6, 1/6, 2/6... up to 6/6)
      const targetIdx = Math.round((index * (totalItems - 1)) / 6);
      const bucket = data[targetIdx];
      const dateVal = new Date(bucket.key_as_string);

      // Render full Month + Day on the first tick to match standard Kibana context
      const timeString = index === 0
        ? dateVal.toLocaleString([], { month: "short", day: "numeric", hour: "2-digit", minute: "2-digit", second: "2-digit" })
        : dateVal.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit", second: "2-digit" });

      return (
        <span key={index} className="truncate max-w-[75px] sm:max-w-none">
          {timeString}
        </span>
      );
    });
  ...
  return (
    <div className="w-full bg-white rounded-xl border border-slate-200 p-2 shadow-sm flex flex-col gap-1 select-none">
      ...
      {/* 7-Point Axis Labels Timeline Footer */}
