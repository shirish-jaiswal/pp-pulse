...
const buildRequestPayload = (data: MatchPhraseData) => {
  const payload: any = {
    ...
    sort: [
      {
        "@timestamp": {
          order: data.sortOrder, // Dynamically set sort direction
        },
      },
      {
        "_doc": {
...
const calculateOptimalInterval = (fromStr: string, toStr: string): { key: "fixed_interval" | "calendar_interval"; value: string } => {
