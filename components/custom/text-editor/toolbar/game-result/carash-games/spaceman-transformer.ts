...
export function transformSpacemanToConfig({ roundDetails }: SpacemanTransformerProps) {
  ...
  const sections = validBets.map((validatedBet, index) => {
    ...
    // --- Core Multiplier Rules Integration ---
    const isHalfOptedIn = validatedBet.halfmultiplier && validatedBet.halfmultiplier > 0;
    ...
    return {
      ...
      hc: {
        ...
        optedAt: isHalfOptedIn && halfTargetConfig ? `${halfTargetConfig}x` : "Not Opted",
        ...
        isOptedIn: isHalfOptedIn
      },
      co: {
        typeName: cashOutTypeDisplay.toUpperCase(),
        optedAt: isCompleteOptedIn && fullTargetConfig ? `${fullTargetConfig}x` : "Not Opted",
